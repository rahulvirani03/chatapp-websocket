import { Container, FlexBetween, FlexContainer, Text } from "@components/custom";
...
useEffect(()=>{
  ...
  const SelfTile = ({item}) => {
    return (
      <FlexContainer style={{height:"100%",alignItems:"center", padding:"auto",marginTop:"auto"}}>
        <Avatar
          size={"large"}
          shape="circle"
          style={{
  ...
  const OtherTile = ({item}) => {
    ...
    return (
      <FlexBetween style={{height:"100%",alignItems:"center", padding:"auto",marginTop:"auto"}} >
<FlexContainer style={{height:"100%",alignItems:"center", padding:"auto",marginTop:"auto"}}> 
        <Avatar
          size={"large"}
          shape="circle"
        ...
        {item.lastMessage.isRead ?  <div>
          ...
          <div>
          {item.user1.id === user.currentUser?.id ? (
            <Text style={{ fontWeight:"bold",marginLeft: "1rem" }}>{item.user2.username}</Text>
          ) : (
            <Text style={{fontWeight:"bold",marginLeft: "1rem" }}>{item.user1.username}</Text>
          )}
          {item.lastMessage.message && (
            <FlexContainer style={{ marginLeft: ".5rem" }}>
              <span style={{ margin: "0px" }}>
              ...
              <span style={{ marginLeft: "4px", marginTop: "0px" ,fontWeight:"bold"}}>
                {item.lastMessage?.message}
              </span>
            </FlexContainer>
          )}
      ...
      {!item.lastMessage.isRead ?   <div style={{height:"10px",width:"10px",borderRadius:"50%",background:`${colors.primary}`}}></div> :<></>}
      ...
      <List itemLayout="vertical">
        {tempChatList?.map((item) => {
          return (
            ...
            >
              {item.lastMessage.from===user.currentUser.id? <SelfTile item={item}/>:<OtherTile item={item}/>}
            </Container>
          );
        })}
      </List>
