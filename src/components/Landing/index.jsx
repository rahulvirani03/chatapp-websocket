...
const location = useLocation();
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      setLoading(true);
      const res = await user.SignUpUser(name, email.trim(), password.trim());
      console.log(res);
      setLoading(false);
  ...
  return (
    ...
    <Card style={{ maxWidth: 500, width: "100%", position: "relative" }}>
      <Tabs defaultActiveKey="1" >
        <TabPane   disabled={loading} tab={<strong style={{color:`${colors.primary}`}}>Login </strong>}  key="1">
          <Form
            requiredMark={false}
            name="login"
            form={loginForm}
        ...
        <TabPane disabled={loading} tab={<strong style={{color:`${colors.primary}`}}>Signup</strong>} key="2">
          <Form
            requiredMark={false}
            form={signupForm}
    ...
    // <div>
    //   <AuthOverlay loading={loading} loginForm={loginForm} signupForm={signupForm} handleLogin={handleLogin} handleSignup={handleSignup} user ={user}/>    
    // </div>
  );
}
